...
(comment
  ...
  #_(defn upper-bound-v2
      ([tolerance-key tolerances]
       (get-in tolerances
               [tolerance-key :high]))
      ([tolerance-key] (upper-bound-v2 tolerance-key tolerances)))
...
(defn co2-tolerable?-refactored
  ...
  (when (:carbon-dioxide atmosphere)
    (<= (lower-bound :co2)
...
(defn surface-temp-tolerable?-refactored
  [{surface-temp-deg-c :surface-temp-deg-c}]
  (let [temp surface-temp-deg-c
        low  (:low temp)
        high (:high temp)]
    (when (and low high)
      (<= (lower-bound :surface-temp-deg-c)
...
(defn surface-temp-tolerable?-refactored-v2
  [{{:keys [low high] :as surface-temp-deg-c} :surface-temp-deg-c
    :as planet}]
  (when (and low high)
    (<= (lower-bound :surface-temp-deg-c)
        low
