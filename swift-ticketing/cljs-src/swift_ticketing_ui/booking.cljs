...
(defn booking-page [booking-id]
  (let [loading (r/atom true)
    ...
    (fn [booking-id]
      @booking-status
      (do
        (when (or (= "InProcess" @booking-status) (= "PaymentPending" @booking-status) (nil? @booking-status))
          (js/setTimeout
           (fn [] (get-booking-status (str "http://127.0.0.1:9090/booking/" booking-id "/status") handler))
           3000))
